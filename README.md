...
The chart is generated by an external star-history service. It can show live repository star data when the repository is public; private repositories may not expose data to the chart service. This README uses the PNG endpoint instead of the SVG endpoint because the SVG embeds a remote GitHub avatar that can appear as a broken image in GitHub README rendering.
